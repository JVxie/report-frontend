...
layui.define(['table', 'form'], function(exports){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{title: '操作', fixed: 'right', toolbar: '#table-admin-user-list', unresize: true, minWidth: 225, align: 'center'}
        ]]
    ...
    //监听工具条
    table.on('tool(LAY-admin-user-list)', function(obj){
        var data = obj.data;
        if(obj.event === 'del'){
            var nowId = data.id;
            // if (nowId === )
            layer.confirm('确认删除' + data.nickname + '吗？', function(index){
                admin.req({
                    url: setter.userRole.admin.api + '/user/' + nowId
                    ,type: 'DELETE'
                    ,headers: {
                        Authorization: setter.request.tokenPrefix + layui.data(setter.tableName).token
                    }
            ...
            admin.popup({
                ...
                ,success: function(layero, index){
                    view(this.id).render('admin/user/form', data).done(function(){
            ...
            layer.confirm('确认重置' + data.nickname + '的密码吗？<br>该操作会将此用户密码重置为123456', {icon: 3, title:'提示'}, function (index) {
                var nowId = data.id;
                admin.req({
                    url: setter.userRole.admin.api + '/user/' + nowId + '/reset_password'
                    ,type: 'POST'
                    ,headers: {
                        Authorization: setter.request.tokenPrefix + layui.data(setter.tableName).token
                    }
    ...
    // // 用户列表（多选)
    // table.render({
    //     elem: '#LAY-admin-user-checkbox-list'
    //     ,url: setter.userRole.admin.api + '/user'
    //     ,headers: {
    //         'Authorization': setter.request.tokenPrefix + layui.data(setter.tableName).token
    //     }
