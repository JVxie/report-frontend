...
layui.define(['table', 'form'], function(exports){
    ...
    //班级列表
    table.render({
        elem: '#LAY-teacher-class-list'
        ,url: setter.userRole.teacher.api + '/class'
        ,headers: {
        ...
        ,cols: [[
            ...
            ,{title: '操作', fixed: 'right', toolbar: '#table-teacher-class-list', minWidth: 100, unresize: true, align: 'center'}
        ]]
    ...
    //监听工具条
    table.on('tool(LAY-teacher-class-list)', function(obj){
        let data = obj.data;
        if(obj.event === 'check'){
            admin.popup({
                title: '查看班级信息'
                ,area: ['400px', '450px']
                ,id: 'LAY-teacher-class-update'
                ,success: function(layero, index){
                    view(this.id).render('teacher/class/form', data).done(function(){
                        ...
                        form.on('submit(LAY-admin-class-submit)', function(data){
                            ...
                            admin.req({
                                url: setter.userRole.teacher.api + '/class/' + nowId
                                ,type: 'PUT'
                                ,headers: {
                                    Authorization: setter.request.tokenPrefix + layui.data(setter.tableName).token
                                }
                                ...
                                ,done: function(res) {
                                    ...
                                    layui.table.reload('LAY-teacher-class-list'); //重载表格
    ...
    //班级列表（单选）
    table.render({
        elem: '#LAY-teacher-class-radio-list'
        ,url: setter.userRole.teacher.api + '/class'
        ,headers: {
            'Authorization': setter.request.tokenPrefix + layui.data(setter.tableName).token
        }
