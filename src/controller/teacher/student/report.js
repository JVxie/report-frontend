...
layui.define(['table', 'form'], function(exports){
    ...
    table.render({
        ...
        ,url: setter.userRole.teacher.api + '/student/report'
        ,headers: {
        ...
        // , toolbar: '#table-teacher-student-list'
        ,cols: [[
            ...
            ,{fixed: 'right', field:'status', title:'填报状态', minWidth:100, templet: '#table-teacher-student-report-status', unresize: true, align: 'center'}
            ,{title: '操作', fixed: 'right', toolbar: '#table-teacher-student-list', minWidth: 85, unresize: true, align: 'center'}
        ]]
    ...
    table.on('tool(LAY-teacher-student-list)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            layer.confirm('确认删除' + data.name + '的今日填报信息吗？', function(index){
                admin.req({
                    url: setter.userRole.teacher.api + '/student/report/' + nowId
                    ,type: 'DELETE'
                    ,headers: {
                        Authorization: setter.request.tokenPrefix + layui.data(setter.tableName).token
                    }
            ...
            admin.popup({
                ...
                ,success: function(layero, index){
                    view(this.id).render('teacher/student/report/form', data).done(function(){
    ...
    exports('teacher/student/report', {})
});