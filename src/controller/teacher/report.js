layui.define(['admin', 'form'], function (exports) {
    ...
    form.on('checkbox(accept)', function ({elem, value, othis}) {
        if (elem.checked) {
            $('#teacher-report-submit').removeClass('layui-btn-disabled').attr("disabled",false);
        } else {
            $('#teacher-report-submit').addClass('layui-btn-disabled').attr("disabled",true);
        }
    });

    form.on('submit(LAY-teacher-report-submit)', function (data) {
        data.field.is_fever_cough_set = Array.from(isFeverCough);
        admin.req({
            url: setter.userRole.teacher.api + '/report'
            , type: 'POST'
    ...
    admin.req({
        ...
        , done: function (res_p) {
            ...
            if (res_p.data.report_today != null) {
                ...
                form.val('layuiadmin-form-teacher', teacher);
                if (reportRecent) {
                    form.val('layuiadmin-form-teacher-report', reportRecent);
                    ...
                    console.log(reportRecent.is_fever_cough_set);
